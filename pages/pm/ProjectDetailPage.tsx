...
const ProjectForm: React.FC<{ project: Project | null, onSuccess: (newProject: Project) => void }> = ({ project, onSuccess }) => {
    ...
    useEffect(() => {
        if (project) {
            setFormData({
                ...
                imageUrl: project.imageUrl ?? null, sections: project.sections || [], managerUserIds: project.managerUserIds || [],
            });
        } else {
    ...
    const managerOptions = useMemo(() => {
        ...
        return opts;
    }, [allEmployeesHook, currentUser]);

    const handleManagerToggle = (uid: string) => {
        if (!canEditDetails) return;
        setFormData(p => {
            const cur = p.managerUserIds || [];
            return { ...p, managerUserIds: cur.includes(uid) ? cur.filter(id => id !== uid) : [...cur, uid] };
        });
    };
    ...
    const translatedTabs = {
        'Detalles': t('project.tab.details'),
        'Programación': t('project.tab.schedule'),
        'Recursos': t('project.tab.resources'),
        'Facturación': t('project.tab.invoicing')
    };
            ...
            <div className="max-h-[65vh] overflow-y-auto pr-2 space-y-4">
                 ...
                 <fieldset className={activeDetailsTab === 'Recursos' ? 'space-y-4' : 'hidden'} disabled={!canEditDetails}>
                    ...
                    <fieldset className="border p-3 rounded dark:border-neutral-600">
                        <legend className="text-xs font-medium px-1">{t('project.resources.add_custom_title')}</legend>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-24 overflow-y-auto bg-neutral-50 dark:bg-neutral-700/50 p-1.5 rounded scrollbar-thin">{allEmployeesHook.filter(emp => (emp as any).userId && canAccessProjects(emp)).map(emp => { const uid = (emp as any).userId as string; return (<label key={emp.id} className="flex items-center space-x-2 text-xs p-1 bg-white dark:bg-neutral-700 rounded cursor-pointer"><input type="checkbox" checked={formData.assignedEmployeeIds.includes(uid)} onChange={() => handleEmployeeToggle(uid)} className="form-checkbox text-primary focus:ring-primary dark:bg-neutral-600 dark:border-neutral-500" /><span>{emp.name} {emp.lastName}</span></label>); })}</div>
                    </div>
...
const ProjectChatView: React.FC<{ project: Project }> = ({ project }) => {
    ...
    // WhatsApp: envío OPT-IN (solo si la tienda tiene WhatsApp activo).
            ...
            <div className="p-3 border-t bg-white dark:bg-neutral-800">
                ...
                {waEnabled && (
                    <label className="flex items-center gap-2 mb-2 text-xs text-neutral-600 dark:text-neutral-300 cursor-pointer select-none">
                        ...
                        {sendToWa && <span className="text-[11px] text-green-600 dark:text-green-400">(irá al chat y a WhatsApp)</span>}
                    </label>
                )}
