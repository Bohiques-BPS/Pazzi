...
export const CategoriesListPage: React.FC = () => {
    ...
    const columns: TableColumn<Category>[] = [
        { header: 'Nombre de Categoría', accessor: 'name', className: 'w-full' },
    ];

    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-semibold text-neutral-700 dark:text-neutral-200">Gestión de Categorías</h1>
            ...
            <DataTable<Category>
                ...
                actions={(category) => (
                    <>
                        <button onClick={() => openModalForEdit(category)} className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 p-1" aria-label={`Editar ${category.name}`}>
                            <EditIcon />
                        </button>
                        <button onClick={() => requestDelete(category.id)} className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 p-1" aria-label={`Eliminar ${category.name}`}>
                            <DeleteIcon />
                        </button>
                    </>
                )}
            ...
            <ConfirmationModal
                ...
                message="¿Estás seguro de que quieres eliminar esta categoría? Esto podría afectar a los productos asociados."
