        ...
        <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-md flex flex-col overflow-hidden hover:shadow-xl dark:hover:shadow-primary/20 transition-shadow duration-200">
            ...
            <div className="p-3 flex flex-col flex-grow">
                <div className="flex justify-between items-start mb-1">
                    <h3 className="text-base font-semibold text-neutral-800 dark:text-neutral-100 line-clamp-2 flex-grow leading-tight">{product.name}</h3>
                    <div className="relative flex-shrink-0 ml-1">
                        ...
                        {actionsOpen && (
                            <div id={`product-actions-${product.id}`} className="absolute right-0 mt-1 w-32 bg-white dark:bg-neutral-700 rounded-md shadow-lg py-1 z-10 border border-neutral-200 dark:border-neutral-600">
                                <button onClick={() => { onEdit(product); setActionsOpen(false); }} className="block w-full text-left px-3 py-1.5 text-xs text-neutral-700 dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-600">Editar</button>
                                <button onClick={() => { onRequestDelete(product.id); setActionsOpen(false); }} className="block w-full text-left px-3 py-1.5 text-xs text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-600/50">Eliminar</button>
                            </div>
                        )}
                    </div>
                </div>
                <p className="text-xs text-neutral-500 dark:text-neutral-400 mb-1 line-clamp-2 flex-grow">{product.description || 'Sin descripción.'}</p>
                <div className="text-xs text-neutral-600 dark:text-neutral-300 mt-auto pt-2 border-t border-neutral-100 dark:border-neutral-700 space-y-0.5">
