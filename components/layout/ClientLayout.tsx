            ...
            <nav className="bg-primary text-white p-4 shadow-md fixed w-full z-20 top-0">
                <div className="container mx-auto flex justify-between items-center">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <button onClick={handleLogout} className="bg-accent hover:bg-amber-600 text-white font-semibold py-1.5 px-3 rounded-md text-sm transition duration-150">
                            Cerrar Sesión
                        </button>
                    </div>
                </div>
                ...
                <aside className={`bg-white dark:bg-neutral-800 text-neutral-700 dark:text-neutral-200 w-64 space-y-2 py-7 px-2 fixed inset-y-0 left-0 top-[64px] transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-200 ease-in-out z-10 shadow-lg border-r border-neutral-200 dark:border-neutral-700`}>
                    <nav className="mt-4">
                        {navLinks.map(link => (
                            link.subLinks ? (
                                <div key={link.name}>
                                    <h3 className="px-4 py-2 text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider flex items-center">
                                    ...
                                    {link.subLinks.map(subLink => (
                                        <Link
                                            ...
                                            className={`block py-2.5 px-4 pl-8 rounded-md transition duration-200 text-sm ${location.pathname === subLink.path ? 'bg-primary text-white' : 'text-neutral-600 dark:text-neutral-300 hover:bg-primary/10 hover:text-primary dark:hover:bg-primary/20 dark:hover:text-white'}`}
                                        >
                                ...
                                <Link
                                    ...
                                    onClick={() => sidebarOpen && setSidebarOpen(false)}
                                    className={`flex items-center py-2.5 px-4 rounded-md transition duration-200 text-sm ${ (location.pathname === link.path || (link.subPathMatch && location.pathname.startsWith(link.subPathMatch))) ? 'bg-primary text-white' : 'text-neutral-600 dark:text-neutral-300 hover:bg-primary/10 hover:text-primary dark:hover:bg-primary/20 dark:hover:text-white'}`}
                                >
