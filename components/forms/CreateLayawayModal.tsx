...
export const CreateLayawayModal: React.FC<CreateLayawayModalProps> = ({
    ...
    const handleSubmit = () => {
        if (!selectedClient) {
            toast.error(t('cmpx.createlayaway.err_client'));
            return;
        }
        if (initialDeposit <= 0) {
            toast.error(t('cmpx.createlayaway.err_deposit_zero'));
            return;
        }
        if (initialDeposit > total) {
            toast.error(t('cmpx.createlayaway.err_deposit_max'));
            return;
        }
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={t('cmpx.createlayaway.title')} size="lg">
            <div className="space-y-4">
                <div className="flex items-center space-x-3 p-2 border rounded-md dark:border-neutral-600">
                    ...
                    <div className="flex-grow">
                        {selectedClient ? (
                            ...
                            <p className="text-sm text-red-500">{t('cmpx.createlayaway.no_client')}</p>
                        )}
                    </div>
                    <button type="button" onClick={onOpenClientSearch} className={BUTTON_SECONDARY_SM_CLASSES}>
                        {selectedClient ? t('cmpx.createlayaway.change') : t('cmpx.createlayaway.search_client')}
                    </button>
                </div>
                
                <div className="p-3 bg-neutral-100 dark:bg-neutral-900 rounded-lg text-center">
                    <p className="text-sm text-neutral-600 dark:text-neutral-300">{t('cmpx.createlayaway.total_label')}</p>
                    <p className="text-3xl font-bold text-primary">${total.toFixed(2)}</p>
                </div>

                <fieldset className="border p-3 rounded-md dark:border-neutral-600">
                    <legend className="text-sm font-medium px-1">{t('cmpx.createlayaway.initial_deposit')}</legend>
                    <div className="flex items-center space-x-4">
                        <div className="flex items-center">
                            ...
                            <label htmlFor="deposit_percentage" className="ml-2 text-sm">{t('cmpx.createlayaway.type_percentage')}</label>
                        </div>
                         <div className="flex items-center">
                            ...
                            <label htmlFor="deposit_fixed" className="ml-2 text-sm">{t('cmpx.createlayaway.type_fixed')}</label>
                        </div>
                    </div>
                    <div className="mt-2">
                        <input
                ...
                <div className="grid grid-cols-2 gap-4 text-center">
                    <div className="p-2 bg-blue-50 dark:bg-blue-900/30 rounded">
                        <p className="text-xs text-blue-800 dark:text-blue-200">{t('cmpx.createlayaway.calculated_deposit')}</p>
                     ...
                     <div className="p-2 bg-yellow-50 dark:bg-yellow-900/30 rounded">
                        <p className="text-xs text-yellow-800 dark:text-yellow-200">{t('cmpx.createlayaway.remaining_balance')}</p>
                        <p className="text-xl font-bold text-yellow-800 dark:text-yellow-200">${remainingBalance.toFixed(2)}</p>
                    </div>
                </div>

                <div>
                    <label className="block text-sm font-medium">{t('cmpx.createlayaway.deposit_method')}</label>
                 ...
                 <div>
                    ...
                    <RichTextEditor value={notes} onChange={setNotes} placeholder={t('cmpx.createlayaway.notes_ph')} />
                </div>
