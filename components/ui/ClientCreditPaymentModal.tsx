...
export const ClientCreditPaymentModal: React.FC<ClientCreditPaymentModalProps> = ({ isOpen, onClose, clientId, clientName, onPaid }) => {
    ...
    const submit = async () => {
        ...
        if (allocations.length === 0) return toast.error('Ingresa un monto a abonar en al menos una factura.');
        setSaving(true);
        try {
            const res = await salesService.bulkPayment({ clientId, method, reference: reference.trim() || undefined, allocations });
            toast.success(`Abono de ${money(res.total)} registrado en ${res.count} factura(s).`);
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Pagos y Créditos — ${clientName || 'Cliente'}`} size="2xl">
            {loading ? (
                <LoadingSkeleton variant="table" rows={5} />
            ) : sales.length === 0 ? (
                <p className="text-center text-neutral-500 py-8">Este cliente no tiene facturas pendientes.</p>
            ) : (
                <div className="space-y-3">
                    <div className="flex items-center justify-between bg-red-50 dark:bg-red-900/20 rounded-md px-4 py-2">
                        <span className="font-medium text-red-700 dark:text-red-300">Balance adeudado</span>
                        <span className="text-xl font-bold text-red-700 dark:text-red-300">{money(totalBalance)}</span>
                    </div>

                    {/* Repartir un total */}
                    <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-sm text-neutral-500">Repartir</span>
                        ...
                        <button type="button" onClick={() => doDistribute(num(distribute))} className={BUTTON_SECONDARY_SM_CLASSES}>Repartir (antiguas primero)</button>
                        <button type="button" onClick={() => doDistribute(totalBalance)} className={BUTTON_SECONDARY_SM_CLASSES}>Pagar todo</button>
                        <button type="button" onClick={() => setAmounts({})} className={`${BUTTON_SECONDARY_SM_CLASSES} !text-red-600`}>Limpiar</button>
                    </div>
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-neutral-100 dark:bg-neutral-700/50 sticky top-0">
                                <tr>
                                    ...
                                    <th className="text-right p-2 w-32">Abonar</th>
                                </tr>
                            </thead>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                            <label className="block text-xs text-neutral-500 mb-1">Referencia (opcional)</label>
                            <input type="text" value={reference} onChange={e => setReference(e.target.value)} placeholder="Nº de confirmación / cheque" className={`${INPUT_SM_CLASSES} w-full`} />
                        </div>
                    </div>

                    <div className="flex items-center justify-between border-t dark:border-neutral-700 pt-3">
                        <span className="text-sm text-neutral-500">Asignado: <b className="text-neutral-800 dark:text-neutral-100">{money(assigned)}</b></span>
                        <div className="flex gap-2">
                            ...
                            <button type="button" onClick={submit} disabled={saving || assigned <= 0} className={`${BUTTON_PRIMARY_SM_CLASSES} disabled:opacity-50`}>
                                {saving ? 'Registrando…' : `Registrar abono ${money(assigned)}`}
                            </button>
                        </div>
                    </div>
                </div>
